...
export default function RealtimeView() {
  ...
  const [currentView, setCurrentView] = useState('sales'); // Toggle: 'sales' or 'expense'
  const [filterType, setFilterType] = useState('Today');
  const [fromDate, setFromDate] = useState('');
  const [toDate, setToDate] = useState('');
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      let url =
        currentView === 'sales'
          ? `http://localhost:5000/api/sales/sales/full-data?userID=${userID}`
          : `http://localhost:5000/api/moneyout/fetch?userID=${userID}`;
      ...
      try {
        ...
        if (response.ok) setData(result.records || []);
      } catch (error) {
    ...
    fetchData();
  }, [currentView, filterType, fromDate, toDate, userID]);
  ...
  return (
    ...
    >
      ...
      <main className="p-6">
        <div className="text-center text-xl font-semibold text-orange-500 mb-4">
          {shopName}
        </div>
        ...
        <div className="flex justify-center mb-6">
          <button
            ...
            className={`px-4 py-2 rounded-l-lg ${
              currentView === 'sales' ? 'bg-orange-500 text-white' : 'bg-gray-200'
            }`}
          >
            Sales Record
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-r-lg ${
              currentView === 'expense' ? 'bg-orange-500 text-white' : 'bg-gray-200'
            }`}
          >
            Expense Record
          </button>
        ...
        <div className="flex flex-wrap items-center justify-center space-x-4 mb-4">
          <input
            ...
            checked={filterType === 'Today'}
          />
          <label className="text-orange-500">Today</label>
          
          <input
            type="radio"
            name="filter"
            onChange={() => setFilterType('Weekly')}
            checked={filterType === 'Weekly'}
          />
          <label className="text-orange-500">Weekly</label>
          ...
          <input
            ...
            checked={filterType === 'Daily'}
          />
          <label className="text-orange-500">Custom</label>
